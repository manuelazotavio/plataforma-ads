...
export default async function ForumPage({
  ...
  return (
    <div className="px-4 md:px-6 py-8 w-full max-w-3xl mx-auto">
        ...
        <div className="divide-y divide-zinc-100">
          {paginated.map((topic) => {
            ...
            return (
              <div
                key={topic.id}
                className="flex items-start gap-3 py-5 sm:gap-5"
              >
                {coverImage && (
                ...
                <div className="flex flex-col gap-1 min-w-0 flex-1">
                  {(cat || isTopOfWeek || isClosed) && (
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {isClosed && (
                        <span className="inline-flex items-center gap-1 rounded-full border border-zinc-200 bg-zinc-50 px-2 py-0.5 text-[10px] font-semibold text-zinc-400">
                  ...
                  <Link href={`/forum/${topic.id}`} className="text-sm font-bold text-zinc-900 leading-snug hover:opacity-70 transition sm:text-base sm:font-black">
                    {topic.title}
                  </Link>
                  <div className="flex items-center justify-between gap-2 mt-0.5">
                    <p className="text-xs text-zinc-400 min-w-0 truncate">
                      ...
                      ) : 'Anônimo'} &bull; {new Date(topic.created_at).toLocaleDateString('pt-BR', { day: 'numeric', month: 'short' })}
                    </p>
