...
export default function NpsSatisfactionPrompt({ userId }: { userId: string | null }) {
  ...
  return (
    <>
      <div className="fixed bottom-20 left-4 z-30 md:bottom-5 md:left-[15rem]">
        <button
          type="button"
          onClick={() => setOpen(true)}
          className="rounded-full border border-zinc-200 bg-white px-4 py-2 text-sm font-semibold text-zinc-700 shadow-lg transition hover:border-[#2F9E41]/40 hover:text-[#2F9E41]"
        >
          {submitted ? 'Atualizar avaliação' : 'Avaliar plataforma'}
        </button>
      </div>

      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4" role="dialog" aria-modal="true" onClick={() => setOpen(false)}>
