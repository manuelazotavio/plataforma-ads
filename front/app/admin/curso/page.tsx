...
export default function AdminCursoPage() {
  ...
  const [scheduleDocs, setScheduleDocs] = useState<ScheduleDoc[]>([])
  ...
  const load = useCallback(async () => {
    const [{ data: settings }, { data: ppcs }, { data: schedules }] = await Promise.all([
      ...
      supabase.from('course_schedule_documents').select('id, label, url, is_active, display_order, created_at').order('display_order', { ascending: false }),
    ])
    ...
    setScheduleDocs((schedules as ScheduleDoc[]) ?? [])
    setLoading(false)
  }, [])
  ...
  async function uploadSchedule(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    e.target.value = ''
    if (!file || !scheduleLabel.trim()) return
    ...
    const path = `horarios/horario-${Date.now()}.pdf`
    ...
    const nextOrder = scheduleDocs.length > 0 ? Math.max(...scheduleDocs.map((d) => d.display_order)) + 1 : 1
    const { error: saveErr } = await supabase.from('course_schedule_documents').insert({ label: scheduleLabel.trim(), url: publicUrl, is_active: true, display_order: nextOrder })
    if (saveErr) { setError(saveErr.message) } else {
      setScheduleLabel('')
      const { data } = await supabase.from('course_schedule_documents').select('id, label, url, is_active, display_order, created_at').order('display_order', { ascending: false })
      setScheduleDocs((data as ScheduleDoc[]) ?? [])
      setNotice('Horário adicionado.')
    }
    setUploadingSchedule(false)
  }

  async function toggleScheduleActive(doc: ScheduleDoc) {
    const { error } = await supabase.from('course_schedule_documents').update({ is_active: !doc.is_active }).eq('id', doc.id)
    if (error) { setError(error.message); return }
    setScheduleDocs((prev) => prev.map((d) => d.id === doc.id ? { ...d, is_active: !doc.is_active } : d))
    setNotice(doc.is_active ? 'Horário arquivado.' : 'Horário reativado.')
  }

  async function removeSchedule(doc: ScheduleDoc) {
    if (!(await confirm({ message: `Remover "${doc.label}"?`, confirmLabel: 'Remover' }))) return
    const { error } = await supabase.from('course_schedule_documents').delete().eq('id', doc.id)
    if (error) { setError(error.message) } else { setScheduleDocs((prev) => prev.filter((d) => d.id !== doc.id)); setNotice(`"${doc.label}" removido.`) }
  }
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <div className="flex flex-col gap-5">
        ...
        <section className="rounded-2xl border border-zinc-200 bg-white p-5">
          <h2 className="text-sm font-semibold text-zinc-900 mb-1">Horários de aulas</h2>
          <p className="mt-1 mb-5 text-sm text-zinc-500">Vários horários podem estar ativos ao mesmo tempo. Os arquivados ficam no histórico.</p>

          {scheduleDocs.filter((d) => d.is_active).length > 0 && (
            <div className="mb-4 flex flex-col gap-2">
              <p className="text-xs font-semibold text-zinc-400 uppercase tracking-wide mb-1">Ativos</p>
              {scheduleDocs.filter((d) => d.is_active).map((doc) => (
                <div key={doc.id} className="flex items-center justify-between gap-4 rounded-xl border border-zinc-100 bg-zinc-50 px-4 py-3">
                  ...
                  <div className="flex shrink-0 gap-2">
                    <button type="button" onClick={() => toggleScheduleActive(doc)}
                      className="cursor-pointer rounded-lg border border-zinc-200 px-3 py-1.5 text-xs font-medium text-zinc-600 transition hover:bg-zinc-100">
                      Arquivar
                    </button>
                    <button type="button" onClick={() => removeSchedule(doc)}
                      className="cursor-pointer rounded-lg border border-red-100 px-3 py-1.5 text-xs font-medium text-red-600 transition hover:bg-red-50">
                      Excluir
                    </button>
                  </div>
                </div>
          ...
          {scheduleDocs.filter((d) => !d.is_active).length > 0 && (
            <div className="mb-4 flex flex-col gap-2">
              <p className="text-xs font-semibold text-zinc-400 uppercase tracking-wide mb-1">Histórico</p>
              {scheduleDocs.filter((d) => !d.is_active).map((doc) => (
                <div key={doc.id} className="flex items-center justify-between gap-4 rounded-xl border border-zinc-100 bg-zinc-50 px-4 py-3 opacity-60">
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-zinc-900">{doc.label}</p>
                    <a href={doc.url} target="_blank" rel="noopener noreferrer" className="mt-0.5 block text-xs text-[#2F9E41] hover:underline">Abrir PDF</a>
                  </div>
          ...
          <div className="flex flex-col gap-3 rounded-xl border border-dashed border-zinc-200 p-4 sm:flex-row sm:items-end">
            <label className="flex flex-1 flex-col gap-1 text-xs font-medium text-zinc-500">
              ...
              <input type="text" value={scheduleLabel} onChange={(e) => setScheduleLabel(e.target.value)}
                placeholder="Ex: 1º Período 2026, Todos os semestres"
                className="rounded-lg border border-zinc-300 px-3 py-2 text-sm text-zinc-900 outline-none transition focus:border-zinc-500 focus:ring-2 focus:ring-zinc-200" />
            </label>
            <button type="button" onClick={() => scheduleRef.current?.click()} disabled={uploadingSchedule || !scheduleLabel.trim()}
              ...
              {uploadingSchedule ? 'Enviando...' : 'Selecionar PDF'}
            </button>
